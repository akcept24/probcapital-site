  ...
  return (
    <div style={{ background: "#0F1117", minHeight: "100vh", color: "#F0F2FF" }}>
      ...
      <div style={{ maxWidth: "860px", margin: "0 auto", padding: "120px 24px 80px" }}>
        ...
        {/* Additional contacts + registered office */}
        <div style={{
          marginTop: "32px", background: "rgba(255,255,255,0.02)",
          border: "1px solid rgba(255,255,255,0.07)", borderRadius: "20px", padding: "28px 32px",
        }}>
          <h2 style={{ fontSize: "18px", fontWeight: 700, marginBottom: "20px" }}>
            {ru ? "Специализированные контакты" : "Specialised Contacts"}
          </h2>
          <div style={{ display: "grid", gridTemplateColumns: "repeat(auto-fit, minmax(220px, 1fr))", gap: "16px", marginBottom: "28px" }}>
            {[
              { label: ru ? "Общая поддержка" : "General Support", email: "support@probcapital.com" },
              { label: ru ? "Биллинг и платежи" : "Billing & Payments", email: "billing@probcapital.com" },
              { label: ru ? "Юридические вопросы" : "Legal Inquiries", email: "legal@probcapital.com" },
              { label: ru ? "Конфиденциальность / CCPA" : "Privacy / CCPA", email: "privacy@probcapital.com" },
            ].map((c) => (
              <div key={c.email} style={{ background: "rgba(0,212,170,0.04)", border: "1px solid rgba(0,212,170,0.12)", borderRadius: "12px", padding: "16px 20px" }}>
                <div style={{ fontSize: "11px", color: "#5A6278", fontWeight: 700, letterSpacing: "0.08em", textTransform: "uppercase", marginBottom: "6px" }}>{c.label}</div>
                <a href={"mailto:" + c.email} style={{ color: "#00D4AA", fontWeight: 600, fontSize: "14px", textDecoration: "none" }}>{c.email}</a>
              </div>
            ))}
          </div>
          <div style={{ borderTop: "1px solid rgba(255,255,255,0.06)", paddingTop: "20px" }}>
            <div style={{ fontSize: "11px", color: "#5A6278", fontWeight: 700, letterSpacing: "0.1em", textTransform: "uppercase", marginBottom: "8px" }}>
              {ru ? "Юридический адрес" : "Registered Office"}
            </div>
            <div style={{ color: "#B0B8D1", fontSize: "14px", lineHeight: 1.7 }}>
              ProbCapital LLC · 8 The Green, Suite B · Dover, DE 19901 · USA
            </div>
